...
abstract class OpenSocialRequest {
  ...
  public function getRestRequest($base_url = "") {
    ...
    $request = new OpenSocialHttpRequest($this->rest_method, $url, $this->rest_params, $this->rest_data);
  ...
  public function getRpcBody() {
    $body = array("method" => sprintf("%s.%s", $this->rpc_entity, $this->rpc_operation), 
        "params" => $this->rpc_params, "id" => $this->id);
    return $body;
  }
...
class FetchAppDataRequest extends OpenSocialRequest {
   ...
   * @param string id String identifier for this request.
   */
  public function __construct($user_id, $group_id, $app_id = null, $keys = null, $params = array(), $id = null) {
    parent::__construct($id);
    ...
    // Set up the RPC request.
    $rpc_params = array_merge($params, array("userId" => $user_id, 
        "groupId" => $group_id, "appId" => $app_id, "keys" => $keys));
...
class UpdateAppDataRequest extends OpenSocialRequest {
  ...
  public function __construct($user_id, $data, $id = null) {
    ...
    // Set up the REST request.
    $this->setRequestor($user_id);
    $params = array("fields" => implode(",", $keys));
    $url = sprintf("/appdata/@viewer/@self/@app", $user_id);
    $this->setRestParams("PUT", $url, $params, $app_data->toJsonObject());
    
    // Set up the RPC request.
    $rpc_params = array("userId" => "@viewer", "groupId" => "@self", 
        "appId" => "@app", "data" => $app_data->toJsonObject(), "fields" => $keys);
...
class CreateActivityRequest extends OpenSocialRequest {
  ...
  public function __construct($user_id, $title, $body, $id = null) {
    ...
    // Map the data into app data
    $activity = array("title" => $title, "body" => $body);
